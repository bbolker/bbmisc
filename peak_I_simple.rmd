...
It encapsulates many of the reasons why it's important to take steps to limit the spread of Coronavirus even if we don't think we can stop it from spreading around the world:
even if the epidemic can't be stopped, epidemic control measures could reduce the number of severely ill patients at the peak of the epidemic, so that they can be taken care of by the limited resources (e.g., ICU beds) available.

But if you like to think mechanistically, you may wonder how (or if) it's possible that interventions which can't limit the long-term reach of the epidemic can limit the peak size.
...
This objection is right in the sense that the interventions being contemplated likely need to reduce transmission in order to limit the height of the epidemic peak. And limiting transmission is also expected to limit the total size of the outbreak.

But the reason that #FlattenTheCurve is nonetheless important is that we expect peak height to be far more sensitive to reductions in transmission than total epidemic size.
