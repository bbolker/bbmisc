...
- `greta`: autodiff via PyTorch. Base language R (via Python?). Procedural. [@goldinggreta2019]
...
```{r pkgs, echo = FALSE}
## search for downstream packages by text matching; could do it instead/in addition
## by looking at the reverse dependency graph ...
...
Many of these platforms have converged to using a similar set of computational techniques. Whole books can and have been written about each of these tools, but it may be useful to write a little bit about each one so that users can get a broader sense of what these platforms are doing, and tradeoffs between approaches (e.g. `mgcv` typically uses low-rank approximations while `TMB`/`INLA` rely more on sparse matrices).
...
* Case studies? Revisit examples from @bolkerStrategies2013a?
