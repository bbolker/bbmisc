...
- cumulative distribution function on the original scale: $S(x)$ transformed appropriately. In this case we don't need to mess with derivative scaling.
- density function on the original scale: $S'(x)$ times the base distribution (i.e. the derivative of the base CDF, which is the inverse-link function). Not 100% clear whether it's easier to take numerical derivatives or get them directly from the spline definition, although the latter is certainly more elegant.

What's nice about this approach is that the null case (*no* quantiles given) reverts to the specified baseline distribution, while additional information allows us to deviate from the baseline.
