...
* consider using box-constrained optimization to set bounds on RE standard deviations, dispersion parameters, correlation parameters, zero-inflation parameters, etc. - this will behave better in the case where the parameters converge to the boundaries (e.g. parameterize the negative binomial as $V = \mu(1+\psi \mu)$ and use the `lower` argument to `nlminb` to enforce the constraint $\psi \ge 0$)
