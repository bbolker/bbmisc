...
* **SVG** (Scalable vector graphics): designed for vector-graphics figures. Less common/portable than PDF, but better for editing. Best format if you want to draw graphics in R and export them for manual fine-tuning in a drawing program (e.g. Adobe Illustrator, Inkscape, PowerPoint). In general the objects in your plot will be recognized as objects in the drawing program (although you may have to "ungroup"). Some web browsers might know what to do with it.
* **TikZ**: this is a weird image language built inside of TeX. The big advantage here is that if you put a TikZ-format image inside a (La)TeX document it will be rendered with the same fonts etc. as your main document (and be able to include any strange math symbols you're using). Use the [tikzDevice package](https://CRAN.R-project.org/package=tikzDevice) to export graphics from R in this format. See the [tikz web page](https://tikz.net/) for examples of the crazy things you can do in this format.
* **EPS** (Encapsulated Postscript): an older vector format, an ancestor of PDF. Acts a lot like PDF, but doesn't typically allow transparency in colours (an "alpha channel").

## Raster formats

* **PNG** (Portable network graphics): best overall format for raster images. OK for plots and diagrams, best for serving plots/diagrams on the web (where vector formats are less convenient). Supports transparency. Uses a [compression algorithm](https://www.libpng.org/pub/png/spec/1.2/PNG-Compression.html) that looks for redundant information in sliding windows of pixels: this means that typically you can increase the resolution of rectangular objects in your image while barely increasing the file size. Well-supported in web browsers, etc.
* **JPG**: this format is designed for *photographic* images, not line/area drawings. *You should generally use PNG instead for plots, charts, and line drawings.* JPG will work OK, but its compression algorithm works poorly for line drawings. At high compression or high magnification you'll see speckly compression artifacts around sharp features (such as the edges of letters in text).
...
* You can sometimes embed a raster image *inside* a vector-format file (e.g. one of the features is "a raster of dimensions (h,w) at position (x,y)")
...
* the amazing [ggrastr package](https://CRAN.R-project.org/package=ggrastr) will rasterize (a) specific layer(s) in your ggplot, leaving the other layers in vector format (e.g., you can condense your 10 million overlapping points into a raster while leaving the grid lines, axes labels, etc. in vector format)
