...
(Source code is in [this directory](https://github.com/bbolker/bbmisc/tree/master/sim_sesoi))
...
plotfun(f_lengthen(sim1, nvec))
```

Alternatively:

```{r sim1-plot-stack, fig.width = 8, message = FALSE}
plotfun(f_lengthen(sim1, nvec), stack = TRUE)
```
...
plotfun(f_lengthen(sim2, nvec))
```

```{r sim2-plot-2, fig.width = 8}
plotfun(f_lengthen(sim2, nvec), stack = TRUE)
```
