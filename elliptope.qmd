...
(they also comment that the rounded triangle you get from a projection is "as used in a Wankel engine or a movie projector").
...
fitfun <- function(dd) {
    ...
    fit <- suppressWarnings(suppressMessages(lmer(form, data = dd)))
    return(fit)
}
...
if (file.exists(fn)) {
    ...
    save(list=paste0("sims", 1:3), file = fn)
}
```
