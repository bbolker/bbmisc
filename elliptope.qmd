...
```{r}
det_comb <- transform(det_dat,
                     detcat = cut(log10(det + mindet/10),
                                  breaks = c(-Inf, -20, -14, -3, Inf),
                                  labels = c("zero", "small", "medium", "large"))) |>
  bind_cols(purrr::map_dfr(sim_list, get_cor, .id = "sim2")) |>
  as_tibble()
with(det_comb, plot3d(r1, r2, r3, col = as.numeric(detcat)+1))
saveRDS(det_comb, file = "det_comb.rds")
```
...
Trying to figure out which sets of vertices are faces/edges (going up to dim 7 below is memory-hungry ...)

```{r higher-dim, eval = FALSE}
