...
Plot likelihood profile of `lognestSD`:

```{r tmbprofile}
prof <- TMB::tmbprofile(tmbfun_RE, name = "lognestSD",
                        ystep = 0.1,
                        parm.range = c(-2, 2),
                        trace = FALSE, ytol = 3)
plot(prof, type = "b")
...
Try a simulation exercise:

```{r}
set.seed(101)
par00 <- round(2*tmbfit_RE$par)/2 ## 'rounded' (to nearest 0.5) versions of tmbfit_RE$par
simfun <- function(par = par00) {
  debug(tmbfun_RE$simulate)
  tmbfun_RE$simulate(par00)
##  simdata <- transform(dat2S, Surv = 
}
NULL
```
...
```{r incomplete-simfun}
powerlogist_sim <- function(x, beta = c(1,2)) {
    mu <- with(data, 
               mu <- plogis(beta[1]+beta[2]*x)^exposure
               )
    y <- rbinom(nrow(data), size = 1, prob = mu)
    data$y <- y
    return(data)
}
```
