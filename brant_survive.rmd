---
title: "Logistic regression, accounting for differences in exposure"
author: "Ben Bolker"
...
This is a general document, originally inspired by one question about 
nest survival and another about tree survival, about analyzing
survival data, possibly in discrete time, possibly with random effects and/or "frailty" (individual-level variation in survival probability).  In particular, I'm interested in doing it efficiently (both in terms of computation time and in terms of compact code) in R, or at least from R (e.g. in AD Model Builder or in WinBUGS or TMB or Stan ...)
...
To use this approach in R, use `family=binomial(link="cloglog")` and add a term of the form `offset(log(A))` to the 
formula (some modeling functions take `offset` as a separate
argument).

Also note that if you allow `log(A)` to be a *covariate* (predictor, independent variable, whatever you want to call it) instead of an offset, then we have a linearly increasing or decreasing log-hazard → a hazard that is a power-law function of exposure → a [Weibull distribution](https://en.wikipedia.org/wiki/Weibull_distribution) for the distribution of survival times (when the coefficient is 1, i.e. if we use an offset, this reduces to an exponentially distributed survival time).
...
```{r opts_pkgs,echo=FALSE,message=FALSE}
pkgs <- sort(c("reshape2","lme4",
               "plyr","bbmle",
               "visreg","emmeans","sjPlot",
               "effects", "RTMB",
               "broom","broom.mixed","ggplot2",
               "dotwhisker"))
...
This was previously done with AD Model Builder, but is much easier in RTMB. Setting up the model is slightly more complicated in RTMB (below I've made it slightly more complex than absolutely necessary, for generality).
...
In principle, we should be able to implement this model with random effects in either `lme4` or in RTMB.  (Doing it in `bbmle` is essentially impossible.)
...
par0_RE <- list(beta=unname(tmbfit$par), b = rep(0, ncol(Z)), log_nestSD = 0)
nllfun(par0_RE)
...
If we have multiple observations per individual (i.e. each observed individual is) we should really *not* be treating each observation
separately.  Adding a random effect of individual might help a little but is papering over the cracks.
