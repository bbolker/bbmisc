...
* if you're using base-R graphics, consider using `par(las=1)` to make the y-axis tick labels horizontal (you may have to adjust the spacing/distance of the y-axis label from  the y-axis)
...
* it's OK to cite preprints if that is the only available form of a paper, but the published version is better if one exists

(see also reference stuff in [peeves](https://bbolker.github.io/bbmisc/peeves.html))
...
* unless you intend to start a new paragraph, don't follow a display equation with a blank line -- LaTeX will think you want a new paragraph and indent the next line. For readability, you can include a line consisting only of a comment character `%`.
...
* If you have code in your thesis, it should be open and fully reproducible (with some exceptions allowed for sensitive data, etc.).
* GitHub is not an archival site (Microsoft could shut it down or change the terms of service in the future): however, [Zenodo](https://zenodo.org/) is an archival site run by CERN that connects directly to GitHub. If you sign up for a Zenodo account, you can then link your GitHub account to it and archive the latest version of your code by clicking a single button. Zenodo will then provide a DOI which you should include in the text of your thesis. (Alternately, you can upload files directly to Zenodo.) If you have a different preferred archive site (e.g. OSF), you're welcome to use that instead.

## language peeves

I have many preferences about language and style, see [here](https://bbolker.github.io/bbmisc/peeves.html) for a complete list.
...
- horn-blowing: "importantly", "interestingly". Your writing should be spare enough that you don't need to emphasize particular results.
